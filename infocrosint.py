...
def info_dominio(dominio):
    ...
    builtwith.analyze_builtwith(config.BUILTWITH_API_KEY , dominio)
    clearbit.analyze_clearbit(config.CLEARBIT_SK, dominio, t)
...
def ayuda():
  ...
  ######################################################################
    Usage: python3 infocrosint.py [OPTIONS]
